...
// ==========================================================
// IMPORTANT: Global Variables Canvas environment se (Agar ye file Node/Webpack mein chalegi to inko
// process.env se access karna hoga, ya ye global variables define hone chahiye)
// ==========================================================
...
export default function App() {
    ...
    // Prompt Copy Function (Using modern Clipboard API)
    const copyPrompt = useCallback((text) => {
    ...
    if (loading) {
        return (<div className="min-h-screen flex items-center justify-center bg-gray-900 text-white"><div className="flex items-center space-x-2"><Loader2 size={32} className="animate-spin text-purple-500" /><p className="text-lg">Database se jood raha hai...</p></div></div>);
    }

    if (error) {
        return (<div className="min-h-screen flex flex-col items-center justify-center bg-gray-900 text-white p-4"><AlertTriangle size={48} className="text-red-500 mb-4" /><h1 className="text-xl font-bold mb-2">Galti (Error)</h1><p className="text-red-300 text-center mb-4">Aapka app database se jud nahi paya. Kripya neeche dekhein:</p><div className="bg-gray-800 p-4 rounded-lg text-sm max-w-lg w-full break-words"><h2 className="font-semibold text-yellow-400 mb-1">Detailed Error:</h2><p className="text-gray-300">{error}</p></div><p className="text-sm text-gray-500 mt-4">Agar yeh error lagatar aata hai toh kripya browser Console mein Red errors (agar hain) zaroor check karein.</p></div>);
    }
            ...
            <header className="bg-gray-800 border-b border-gray-700 sticky top-0 z-10">
                <div className="max-w-6xl mx-auto px-4 py-4 flex justify-between items-center">
                    <div className="flex items-center space-x-2">
                        <div className="bg-purple-600 p-2 rounded-lg"><ImageIcon size={24} /></div>
                        <h1 className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-purple-400 to-pink-600">PromptMaster (LIVE Data)</h1>
                    </div>
